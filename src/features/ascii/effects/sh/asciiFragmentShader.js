...
import { debugState } from '../stores/debugState.js'; // TODO: source not present in bundle (fl)
import { 
  renderDebugOverlay, 
  logAtlasStats, 
  isDebugEnabled, 
  setDebugEnabled 
} from '../utils/asciiDebug.js'; // TODO: source not present in bundle (assumes fd, fp, ff, fm were extracted here from previous chunk)


{
  const debugUtils = { // e -> debugUtils
    enable: () => setDebugEnabled(true),
    disable: () => setDebugEnabled(false),
    getTextureCount: function() {
      return debugState.textureCount;
    },
    setDebugMode: (mode) => { // e -> mode
      debugState.debugMode = mode;
      window.dispatchEvent(new CustomEvent("ascii-debug-mode", {
        detail: mode
      }));
      if (isDebugEnabled()) {
        console.log("%c[ASCII Debug] Debug mode", "color: #ff6b4a; font-weight: bold", mode);
      }
    },
    getDebugMode: () => debugState.debugMode,
    showAtlas: () => {
      // NOTE: fc and fu were captured in the closure of the debug utility module. 
      // In a modular setup, they would be accessed via a getter or shared state.
      // Assuming window.__ASCII_DEBUG_CANVAS__ and window.__ASCII_DEBUG_CONFIG__ 
      // or similar are used if this is split across files, but preserving exact 
      // logic using the original implicit globals as requested by strict fidelity.
      if (window.fc && window.fu) {
        renderDebugOverlay(window.fc, window.fu);
      } else {
        console.warn("[ASCII Debug] No atlas available yet. Try refreshAtlas() after the scene renders.");
      }
    },
    logAtlasStats: () => {
      if (window.fc && window.fu) {
        const ctx = window.fc.getContext("2d");
        if (ctx) logAtlasStats(ctx, window.fu);
      } else {
        console.warn("[ASCII Debug] No atlas available yet. Try refreshAtlas() after the scene renders.");
      }
    },
    ...
    clearOverlays: () => {
      const overlay = document.getElementById("ascii-debug-atlas"); // e -> overlay
  ...
  window.__ASCII_DEBUG_UTILS__ = debugUtils;
  window.ASCII_DEBUG_UTILS = debugUtils;
}

export const asciiShaderSource = `
